...
Future<List<Announcement>> fetchAnnouncements() async {
  final response = await http.get(Uri.parse('${loginPage.apiUrl}paannouncements'));
...
Future<void> _deletenews(BuildContext context, String pid) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      await Navigator.push(context, MaterialPageRoute(builder: (context) => coonAnnouncementList()));
    } else {
...
class coonAnnouncementList extends StatefulWidget {
  const coonAnnouncementList({super.key});

  @override
  State<coonAnnouncementList> createState() => _AnnouncementListState();
}

class _AnnouncementListState extends State<coonAnnouncementList> {
